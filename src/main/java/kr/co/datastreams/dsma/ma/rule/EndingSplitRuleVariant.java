 ...
 * '어'와 결합되는 어미
 */
public class EndingSplitRuleVariant extends BaseEndingRule {

    private char[] jungsungCheck = {'ㅏ','ㅓ', 'ㅔ', 'ㅐ'};
    public EndingSplitRuleVariant(String stemCandidate, String endingCandidate) {
    ...
    public boolean canHandle() {
        ...
        if (phonemes.containsJungsung(jungsungCheck)) {
            ...
            if (!phonemes.hasJong() || SyllableDic.getFeature(endOfStemCandidate)[SyllableDic.IDX_YNPAH] == '1') {
                if (EomiDic.findEndingWith('어', endingCandidate) != null) {
                    return true;
                }
            }
        }
    ...
    public Variant split() {
        ...
        if (phonemes.hasJong()) {
            return Variant.createEnding(stemCandidate, endingCandidate);
        } else {
