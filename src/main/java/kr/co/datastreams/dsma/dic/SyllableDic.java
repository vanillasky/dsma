...
public class SyllableDic implements ConfKeys {
    ...
    public static boolean isSecondJosaSyllable(char ch) {
        byte[] features = getFeatureInByte(ch);
        return features[IDX_JOSA2] == 1;
    }
    ...
    public static boolean hasOnlyVerbSyllable(String word) {
        for (int i=word.length()-1; i >= 0; i--) {
            byte[] features = getFeatureInByte(word.charAt(i));
